...
asmlinkage void handle_unaligned_c(struct pt_regs *fp)
{
	...
	if (in_kernel || (ma_usermode & UM_FIXUP)) {
		...
		case inst_sth:
			...
			d0 = val >> 0;
#if 0
				printk("sth: ra=%d (%08x) rb=%d (%08x), "
				       "imm16 %04x addr %08x val %08x\n",
				       a, get_reg_val(fp, a),
				       b, get_reg_val(fp, b),
				       imm16, addr, val);
#endif
	...
	if (fault) {
		if (in_kernel) {
			printk(KERN_ERR "fault during kernel misaligned fixup @ "
			       "%#lx; addr 0x%08lx; isn=0x%08x\n",
			       fp->ea, (long unsigned int)addr,
			       (unsigned int)isn);
		} else {
			printk(KERN_ERR "fault during user misaligned fixup @ "
			       "%#lx; isn=%08x addr=0x%08x sp=0x%08lx pid=%d\n",
			       fp->ea,
	...
	if (in_kernel) {
		...
		if (ma_usermode & KM_WARN) {
			printk(KERN_ERR "kernel unaligned access @ %#lx; "
			       "BADADDR 0x%08lx; cause=%d, isn=0x%08lx\n",
			       fp->ea,
	...
	if (ma_usermode & UM_WARN) {
		printk(KERN_ERR "user unaligned access @ %#lx; "
		       "isn=0x%08lx ea=0x%08lx ra=0x%08lx sp=0x%08lx\n",
